"""Tests for probpipe._array_utils helpers."""
...
class TestArrayUtils:
    """Coercion / shape-validation helpers used across the core library."""

    def test_ensure_real_scalar_from_python_scalar(self):
    ...
    def test_ensure_real_scalar_from_numpy_scalar_and_0d(self):
        a = np.float32(2.0)
    ...
    def test_ensure_real_scalar_rejects_multiple_elements(self, non_scalar_input):
        with pytest.raises(ValueError):
            U._ensure_real_scalar(non_scalar_input)

    def test_ensure_real_scalar_rejects_complex(self):
        with pytest.raises(ValueError):
            U._ensure_real_scalar(1 + 2j)
        with pytest.raises(ValueError):
            U._ensure_real_scalar(np.array(1 + 0j))

    def test_ensure_batch_real_scalar_single_and_array(self):
    ...
    def test_ensure_vector_scalar_and_1d_and_2d(self):
    ...
    def test_ensure_vector_length_check(self):
        with pytest.raises(ValueError):
            U._ensure_vector([1, 2, 3], length=2)

    def test_ensure_matrix_scalar_1d_2d(self):
    ...
    def test_ensure_matrix_rejects_ndim_gt3(self):
        with pytest.raises(ValueError):
            U._ensure_matrix(np.zeros((2, 2, 2)))
        with pytest.raises(ValueError):
            U._ensure_matrix(np.ones((1, 1, 1, 1)))

    def test_ensure_matrix_dim_checks(self):
        with pytest.raises(ValueError):
            U._ensure_matrix(2, num_rows=1, num_cols=2)
        with pytest.raises(ValueError):
            U._ensure_matrix([1, 2], as_row_matrix=True, num_rows=2)
        with pytest.raises(ValueError):
            U._ensure_matrix([1, 2], as_row_matrix=False, num_cols=2)
    ...
    def test_ensure_batch_array_basic_and_value_shape(self):
        a = np.array([1, 2, 3])
    ...
    def test_ensure_batch_real_scalar(self):
        with pytest.raises(ValueError):
            U._ensure_batch_real_scalar(np.zeros((2, 2)))

    def test_ensure_batch_vector(self):
    ...
    def test_ensure_batch_matrix(self):
    ...
    def test_copy_semantics_ensure_vector_matrix_batch(self):
        arr = np.array([1.0, 2.0, 3.0])
        v_copy = U._ensure_vector(arr, copy=True)
        assert v_copy is not arr
