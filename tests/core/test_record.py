...
class TestConstruction:
    def test_kwargs(self):
        v = Record("r", r=1.8, K=70.0, phi=10.0)
    ...
    def test_positional_accepts_any_mapping(self):
        ...
        v = Record("r", MappingProxyType({"a": 1.0, "b": 2.0}))
        assert v.fields == ("a", "b")
        nr = NumericRecord("nr", OrderedDict([("z", jnp.zeros(2)), ("a", 1.0)]))
        assert nr.fields == ("z", "a")  # mapping iteration order preserved

    def test_insertion_order_preserved(self):
        v = Record("r", z=1.0, a=2.0, m=3.0)
...
class TestFieldAccess:
    @pytest.fixture
    def v(self):
        return Record("r", r=1.8, K=70.0, phi=10.0)

    def test_item(self, v):
        np.testing.assert_allclose(float(v["K"]), 70.0, rtol=1e-5)

    def test_key_path_tuple(self):
        v = Record("r", params=Record("r", r=1.8, K=70.0), obs=Record("r", y=np.zeros(5)))
    ...
    def test_key_path_string_three_levels(self):
        v = Record("r", a=Record("r", b=Record("r", c=42.0)))
        assert v["a/b/c"] == 42.0

    def test_path_in_membership(self):
        v = Record("r", params=Record("r", r=1.8, K=70.0))
...
class TestImmutability:
    ...
    def test_replace_nested_path(self):
        v = Record("r", physics=Record("r", force=1.0, mass=2.0), obs=3.0)
    ...
    def test_merge_preserves_numeric_record(self):
        ...
        nr = NumericRecord("nr", a=1.0)
        assert type(nr.merge(NumericRecord("nr", b=2.0))) is NumericRecord
...
class TestKeysAgreement:
    ...
    def test_flat(self):
        v = Record("r", a=1.0, b=jnp.zeros(3), c="x")
        assert list(v.keys()) == list(v.event_template.keys())

    def test_nested_in_family(self):
        v = Record("r", theta=Record("r", loc=jnp.zeros(2), s=jnp.ones(3)), top=jnp.array(1.0))
        assert list(v.keys()) == list(v.event_template.keys())
...
class TestPyTree:
    ...
    def test_tree_leaves(self):
        v = Record("r", a=jnp.array(1.0), b=jnp.array(2.0))
        leaves = jax.tree.leaves(v)
        assert len(leaves) == 2

    def test_tree_structure_roundtrip(self):
        v = Record("r", x=jnp.array([1.0, 2.0]), y=jnp.array(3.0))
    ...
    def test_nested_tree_map(self):
        v = Record("r", params=Record("r", r=1.0, K=2.0), z=3.0)
    ...
    def test_vmap(self):
        batch = Record("r", x=jnp.array([1.0, 2.0, 3.0]))
...
class TestConversion:
    def test_to_dict_verbatim(self):
        ...
        v = Record("r", a=1.0, b=arr, label="tag")
        d = v.to_dict()
    ...
    def test_to_numpy_preserves_opaque(self):
        v = Record("r", label="x", y=np.array([1.0, 2.0]))
        d = v.to_numpy()
        assert d["label"] == "x"
        assert isinstance(d["y"], np.ndarray)

    def test_to_dict_nested(self):
        v = Record("r", inner=Record("r", x=1.0), y=2.0)
        d = v.to_dict()
    ...
    def test_to_numeric_returns_numeric_record(self):
        ...
        v = Record("r", a=1.0, b=jnp.array([2.0, 3.0]))
        nr = v.to_numeric()
...
class TestReprAndEquality:
    ...
    def test_hash_excludes_value(self):
        ...
        v1 = Record("r", a=jnp.zeros(3))
        v2 = Record("r", a=jnp.ones(3))
        assert hash(v1) == hash(v2)

    def test_hash_distinguishes_dtype(self):
        """Records with the same shape but different dtype hash differently."""
        v1 = Record("r", a=jnp.zeros(3, dtype=jnp.float32))
        v2 = Record("r", a=jnp.zeros(3, dtype=jnp.int32))
        assert hash(v1) != hash(v2)
...
class TestProvenance:
    ...
    def test_replace_resets_provenance(self):
        r = Record("r", x=1.0).with_provenance(Provenance("orig", parents=()))
        r2 = r.replace(x=2.0)
    ...
    def test_merge_resets_provenance(self):
        r = Record("r", x=1.0).with_provenance(Provenance("orig", parents=()))
        merged = r.merge(Record("r", y=2.0))
        assert merged.provenance is None

    def test_without_resets_provenance(self):
        r = Record("r", x=1.0, y=2.0).with_provenance(Provenance("orig", parents=()))
        r2 = r.without("y")
        assert r2.provenance is None
    ...
    def test_eq_ignores_provenance(self):
        r1 = Record("r", x=1.0).with_provenance(Provenance("a", parents=()))
        r2 = Record("r", x=1.0).with_provenance(Provenance("b", parents=()))
        assert r1 == r2
    ...
    def test_pytree_roundtrip_drops_provenance(self):
        r = Record("r", x=1.0, y=jnp.array([2.0, 3.0]))
...
class TestEventTemplateStorage:
    ...
    def test_nested_field_name_mismatch_raises_with_path(self):
        ...
        with pytest.raises(ValueError, match="physics"):
            Record(
                "r",
                physics=Record("physics", force=1.0, momentum=2.0),  # wrong nested field name
                obs=jnp.zeros(5),
                event_template=tpl,
            )
