...
class TestDenseLinOp:
    ...
    def test_array_properties(self):
        ...
        assert jnp.array_equal(self.op.to_dense(), self.arr)

    def test_transformation_views(self):
        assert jnp.array_equal(self.op.T.to_dense(), self.arr.T)
        assert jnp.array_equal((2 * self.op).to_dense(), 2 * self.arr)
        assert jnp.array_equal((self.op + self.op).to_dense(), self.arr + self.arr)
        assert jnp.array_equal((self.op @ self.op).to_dense(), self.arr @ self.arr)

    def test_linalg_operations(self):
        L = self.op.cholesky(lower=True).to_dense()
        assert approx(L @ L.T, self.arr)
        assert jnp.array_equal(self.op.trace(), jnp.trace(self.arr))
        assert approx(self.op.logdet(), jnp.linalg.slogdet(self.arr)[1])
        assert jnp.array_equal(self.op.diag(), jnp.diag(self.arr))

        b1 = jnp.ones((self.n,))
        b2 = jnp.ones((self.n, 1))
        B = jnp.ones((self.n, 4))
        assert approx(self.op.solve(b1), jnp.linalg.solve(self.arr, b1))
        assert approx(self.op.solve(b2), jnp.linalg.solve(self.arr, b2))
        assert approx(self.op.solve(B), jnp.linalg.solve(self.arr, B))
    ...
    def test_matvec_matmat(self):
        ...
        assert jnp.array_equal(self.op.matvec(b1), self.arr @ b1)
        assert jnp.array_equal(self.op.matvec(b2), (self.arr @ b2).ravel())
        assert jnp.array_equal(self.op.matmat(b1), (self.arr @ b1).reshape(-1, 1))
        assert jnp.array_equal(self.op.matmat(b2), self.arr @ b2)
        assert jnp.array_equal(self.op.matmat(B), self.arr @ B)
...
def test_diagonal_matvec_solve_cholesky_flags():
    ...
    b = jnp.array([2.0, 6.0])
    sol = d.solve(b)
    assert approx(sol.squeeze(), jnp.array([1.0, 2.0]))
    # cholesky of diagonal is diag(sqrt)
    L = d.cholesky()
    assert isinstance(L, DiagonalLinOp)
    assert approx(L.diag(), jnp.sqrt(diag))
...
def test_sum_propagation_and_matmat():
    ...
    d2 = DiagonalLinOp(jnp.array([3.0, 4.0]))
    s = SumLinOp([d1, d2])
    X = jnp.eye(2)
