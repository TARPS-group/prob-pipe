"""Tests for probpipe.core.values.Values."""
...
class TestConstruction:
    def test_kwargs(self):
        v = Values(r=1.8, K=70.0, phi=10.0)
    ...
    def test_scalars(self):
        v = Values(a=1, b=2.5, c=True)
        assert v.a.shape == ()
        assert float(v.b) == 2.5

    def test_nested(self):
        inner = Values(x=1.0, y=2.0)
        outer = Values(params=inner, z=3.0)
        assert isinstance(outer.params, Values)
        assert float(outer.params.x) == 1.0

    def test_from_dict(self):
        v = Values.from_dict({"a": 1.0, "b": 2.0})
        assert v.fields() == ("a", "b")

    def test_list_input(self):
        v = Values(x=[1.0, 2.0, 3.0])
        assert v.x.shape == (3,)
...
class TestFieldAccess:
    @pytest.fixture
    def v(self):
        return Values(r=1.8, K=70.0, phi=10.0)
    ...
    def test_key_path(self):
        v = Values(params=Values(r=1.8, K=70.0), obs=Values(y=np.zeros(5)))
...
class TestFlatten:
    ...
    def test_flat_size_arrays(self):
        v = Values(x=jnp.zeros(5), y=jnp.zeros((2, 3)))
        assert v.flat_size == 11

    def test_flat_size_nested(self):
        v = Values(params=Values(r=1.0, K=2.0), obs=Values(y=jnp.zeros(4)))
    ...
    def test_flatten_arrays(self):
        v = Values(x=jnp.array([1.0, 2.0]), y=jnp.array([3.0]))
        flat = v.flatten()
        # Sorted: x first, then y
        np.testing.assert_allclose(flat, [1.0, 2.0, 3.0])

    def test_flatten_nested(self):
        v = Values(a=Values(x=1.0, y=2.0), b=3.0)
        flat = v.flatten()
        # a.x=1, a.y=2, b=3
        np.testing.assert_allclose(flat, [1.0, 2.0, 3.0])
    ...
    def test_unflatten_roundtrip(self):
        v = Values(r=1.8, K=70.0, phi=10.0)
        flat = v.flatten()
        v2 = Values.unflatten(flat, template=v)
    ...
    def test_unflatten_nested_roundtrip(self):
        v = Values(
            params=Values(r=1.0, K=2.0),
            obs=Values(y=jnp.array([3.0, 4.0, 5.0])),
        )
        flat = v.flatten()
        v2 = Values.unflatten(flat, template=v)
    ...
    def test_unflatten_preserves_shapes(self):
        v = Values(mat=jnp.zeros((2, 3)), vec=jnp.zeros(4))
        flat = v.flatten()
        v2 = Values.unflatten(flat, template=v)
...
class TestPyTree:
    ...
    def test_tree_leaves(self):
        v = Values(a=jnp.array(1.0), b=jnp.array(2.0))
    ...
    def test_nested_tree_map(self):
        v = Values(params=Values(r=1.0, K=2.0), z=3.0)
        v2 = jax.tree.map(lambda x: x + 10, v)
        assert isinstance(v2, Values)
        assert isinstance(v2.params, Values)
    ...
    def test_vmap(self):
        batch = Values(x=jnp.array([1.0, 2.0, 3.0]))
    ...
    def test_grad(self):
        ...
        grads = jax.grad(f)(v)
        assert isinstance(grads, Values)
...
class TestConversion:
    def test_to_dict(self):
        v = Values(a=1.0, b=np.array([2.0, 3.0]))
        d = v.to_dict()
    ...
    def test_to_dict_nested(self):
        v = Values(inner=Values(x=1.0), y=2.0)
        d = v.to_dict()
...
class TestLeafOps:
    ...
    def test_zip(self):
        v1 = Values(a=1.0, b=2.0)
        v2 = Values(a=10.0, b=20.0)
        zipped = Values.zip(v1, v2)
...
class TestReprAndEquality:
    ...
    def test_repr_arrays(self):
        v = Values(x=jnp.zeros((3, 4)))
        r = repr(v)
        assert "shape=(3, 4)" in r

    def test_repr_nested(self):
        v = Values(inner=Values(x=1.0))
        r = repr(v)
        assert "inner=Values(" in r

    def test_equality(self):
        v1 = Values(a=1.0, b=2.0)
        v2 = Values(a=1.0, b=2.0)
        assert v1 == v2
