...
- **Two-distribution packaging: `probpipe-core` (lean) and `probpipe`
  (core + all backends) (#237).** The root distribution is renamed `probpipe-core` (lean JAX base —
  ...
  are equivalent. (The `pyabc` SMC-ABC backend joins `probpipe` once it lands;
