...
A `Function` keeps two namespaces apart: the wrapped function's arguments, which are every positional and keyword argument of a call, and the framework's **controls**, which select realization, execution, or presentation. The controls are `n_broadcast_samples`, `include_inputs`, `method`, `min_fidelity`, the per-parameter `conversions`, `raw`, and the dispatch and orchestration selectors, together with the numerical budgets a registered method defines. Randomness has no control: no call takes a key or a seed, and every draw's key comes from the workflow scope (IV.8).
...
**Keys.** Every ProbPipe-caused draw is **workflow-owned**: no call takes a key, and each draw's key is derived from the scope. A fixed stream is obtained by fixing the scope's seed, and an implementer's `_sample` receives the derived key (III.8).
