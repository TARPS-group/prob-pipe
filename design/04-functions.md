...
**Structural event identity.** A workflow-owned key is a pure function of the scope's root seed and the event's identity, three structural coordinates: the **occurrence path** — the invocation's position in the workflow, extended by nesting and by managed work items, with repeated or recursive invocations of the same call distinguished by a deterministic logical ordinal; the **stochastic source** — which co-sampling group of IV.2 is drawing; and the **logical unit** — which broadcast repetition or sweep cell consumes the draw. Identity follows the workflow's logical structure: ordinals are fixed by program order, no key is drawn twice, and the same call produces the same draws under any dispatch mode, thread count, or orchestration.
