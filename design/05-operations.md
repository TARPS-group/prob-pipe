...
- With `raw=True`, `sample` skips the wrap. For `sample_shape=()` it returns the bare draw type `T`, carrying no name or provenance, whatever the event type. For a non-empty `sample_shape` it returns `T`'s batch form: an array with the `sample_shape` axes leading when `T` is an `Array`, a `NumericRecordBatch` when `T` is a `NumericRecord` (a `RecordBatch` for a general record), and an element batch otherwise (a `DistributionBatch` for a distribution-valued draw, and a `FunctionBatch` or `OpaqueBatch` for function-valued or opaque draws).
