...
- **The wrap boundary.** The boundary is kind-directed and wraps only the untracked.
  ...
  - *Identity is attached by derivation.* Tracked terms are immutable and provenance is write-once. The default result is therefore a new term: the implementer's representation, shared, under the operation's fresh name and provenance. The implementer's object is never modified. `raw=True` returns it untouched.
  ...
  - *An output declaration is enforced.* When the operation carries one, the result is coerced to the declared kind and validated. A wrong kind and a schema mismatch raise distinct errors. A raw callable is a `Fun` unless the declaration is a `ConditionalDistributionSpec` — the one distinction the value itself cannot reveal.
