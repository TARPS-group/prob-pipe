...
**Routes.** A route is one way to realize an operation. A route has the shape of a dispatch method (II.7) — `check`, `execute`, and a **fidelity** — bound to a call rather than to argument types. Routes come from four sources, and an operation may carry any combination:
