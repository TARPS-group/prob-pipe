...
- **Two levels.** The implementer's `_x` method acts on the implementer draw type `T` (III.7). The user-facing operation wraps the result at the boundary, attaches identity, metadata, and provenance, and broadcasts, all at no cost to the implementer.
...
- **The wrap boundary.** The boundary is kind-directed and wraps only the untracked.
  - *A tracked term keeps its kind.* A `Record`, `Distribution`, `ConditionalDistribution`, `Function`, `NumericArray`, or `Opaque` an operation returns is never re-wrapped and never buried inside another kind. Keeping the kind is not keeping the identity: the default result still carries the operation's fresh name and provenance, by the derivation rule below.
  - *Identity is attached by derivation.* TrackedTerm terms are immutable and provenance is write-once. The default result is therefore a new term: the implementer's representation, shared, under the operation's fresh name and provenance — whichever presentation the implementer returned, a returned tracked term contributing its kind and representation while its own identity stays untouched on the inner term. The implementer's object is never modified. `raw=True` returns it untouched, as produced.
...
- **The `raw` opt-out.** Every operation accepts `raw=True`: the boundary wrap and the identity minting are skipped, and the implementer's result returns as produced. An implementer that produced a bare value returns it bare — the array, the mapping of raw leaves, the callable, the object — and one that produced a tracked term, as `_cov`'s operator or a random measure's draw, returns that term unchanged: nothing is unwrapped to satisfy a kind. Under a non-empty `sample_shape` the raw result is the storage view, the draws' raw values in their native stacked layout (II.7). The wrapped, tracked result is always the default.
- **Four raw mechanisms, one meaning.** *Raw* always means the representation with no ProbPipe extras; the mechanisms differ only in where they act. The implementer methods are *written* over raw types (`T`, III.7); `apply` (III.3) *evaluates* the wrapped callable with no lifting, tracking, or provenance; `raw=True` *skips* an operation's wrap at the boundary; and `raw()` (II.5) *reads* an existing term's stored representation.
