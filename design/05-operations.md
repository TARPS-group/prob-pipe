...
**The evaluation registry.** For a distribution operand the call dispatches through a `BinaryDispatchRegistry` keyed on the map's and the distribution's types, whose methods are **evaluation rules**. The registry is defined beside the engine (Part IV) and consulted by every single-distribution application, the operation and the direct call alike. Auto-selection tries the rules in priority order:
