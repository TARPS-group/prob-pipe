...
1. distinguishes one operand as its **subject**,
2. resolves its implementation from the subject's declared **capabilities**, and
3. declares a **result kind fixed by its operand kinds alone**, never by the runtime value.

Everything else an operation does it does *because it is a `Function`*: controls, provenance, execution dispatch, orchestration, and randomness are Part IV's, not each operation's. That is what keeps the vocabulary small — an operation adds a subject, a capability, and a result kind to machinery that already exists.

**The three laws.** Every operation obeys all three, and a contract below that appears to state otherwise is stating an instance of one.

- **Boundary.** `op(x, raw=True)` is the implementer's result as produced; `op(x)` is that result wrapped at the declared kind under fresh, derived identity. The two therefore agree in value, and `op(x).raw()` recovers what `op(x, raw=True)` returns.
...
- **Closure.** The result is a tracked term whose kind is a function of the operand kinds and the declaration only, so what an operation returns is known before it runs.

**Two species.** Operations divide by what gates them, and the division is what tells a user when a call can fail.

| species | gate | fails when | examples |
|---|---|---|---|
| **capability** | a `Supports*` protocol the subject declares | the subject does not claim the capability, and no fallback is registered | `sample`, `log_prob`, `mean`, `cov`, `quantile`, `marginal`, `factor`, `inverse`, `condition_on` |
| **structural** | none — computed from the operands' declared structure | never, on well-formed operands | `joint` / `*`, `mixture`, `evaluate` |

A **derived** operation is a structural one defined by an identity over other operations rather than by a contract of its own: `mixture` is the detached `marginal` of a composed joint, and inherits its exactness, its slot matching, and its failure behavior from the operations it is defined by. A derived operation adds no new semantics, which is the point of defining it that way.

**Capability dispatch.** A capability operation tests the subject's protocol with `isinstance(obj, SupportsX)` and calls the implementer method, which acts on the implementer draw type `T` (III.7). Where an operation admits several routes — a closed form, a registered rule, a Monte Carlo default — they are ranked by fidelity, and the selected route records its name and fidelity in the result's provenance, so an approximate answer is distinguishable from an exact one after the fact. Each operation declares one **fallback status**: *total by definition* when the operation is the object's own capability, as `sample` is; *fallback-backed* when a default such as Monte Carlo stands behind the closed form, total on a stated domain; and *capability-only* when there is no fallback, as for `log_prob` and `factor`. The status is registry data rather than contract, so registering a numerical default later moves an operation between statuses without changing its signature.

**The wrap boundary.** The boundary is kind-directed and wraps only the untracked.

- *A tracked term keeps its kind.* A `Record`, `Distribution`, `ConditionalDistribution`, `Function`, `NumericArray`, or `Opaque` an operation returns is never re-wrapped and never buried inside another kind. Keeping the kind is not keeping the identity: by the boundary law the result carries the operation's fresh name and provenance, while the implementer's own term is left untouched.
...
**The `raw` opt-out.** Every operation accepts `raw=True`, the boundary law's other side: the wrap and the identity minting are skipped and the implementer's result returns as produced. A bare value returns bare, and a result the implementer produced as a tracked term returns as that term — nothing is unwrapped to satisfy a kind. Under a non-empty `sample_shape` the raw result is the storage view, the draws' raw values in their native stacked layout (II.6).

**Four raw mechanisms, one meaning.** *Raw* always means the representation layer, detached from the workflow; the mechanisms differ only in where they act. The implementer methods are *written* over raw types (`T`, III.7); `apply` (III.3) *evaluates* a wrapped callable with no lifting, tracking, or provenance; `raw=True` *skips* an operation's wrap at the boundary; and `raw()` (II.5) *reads* an existing term's representation.
...
**Randomness.** No operation draws from ambient random state, and no operation carries a seed. An explicit PRNG `key` is caller-owned: the draw is consumed as given and reproducible from the key alone. With the key omitted, the draw is a **workflow-owned random event** (IV.3): its key derives structurally from the enclosing workflow scope, and a bare call outside any scope gets its own ephemeral scope — so an omitted key is never silently repeated, and a seeded scope reproduces it on demand. An operation whose contract is a deterministic quantity but that falls back to Monte Carlo takes its key the same way. The resolved key is recorded in `provenance`.

**Output identity.** Every tracked term an operation mints is fully specified, never left implicit: its spec is carried or derived from the inputs, its `provenance` records the operation and its parent descriptors, and its `name` is auto-derived and marked `name_is_auto`. Every result is tracked, a numeric summary included — a density returns as a `NumericArray` whose provenance records the operation, and the bare value is one `raw=True` away.

### Rationale

Defining an operation by a subject, a capability, and a declared result kind is what makes the vocabulary closed and small: everything else is inherited from `Function`, so adding an operation cannot add a mechanism (`D2 – Generality first`). Capability dispatch is `D3 – Capability-based operations`: a fixed vocabulary applies to any object that mathematically supports it, independent of the object's encoding. The three laws are the user-facing contract — what a call returns, when it lifts, and what `raw` means — and stating them once means no operation may quietly differ. Separating capability from structural operations answers *when can this fail?* with one rule rather than per-operation fine print, and defining a derived operation by an identity keeps its behavior in one place (`D7 – Single source of truth`). The implementer/user split serves `C3 – Computational detail hidden by default, available on demand` and `C6 – Traceable and reproducible workflows` together: an implementer writes the mathematics in plain arrays while the user receives a tracked, named result with its route recorded. That every operation returns another tracked term is `D4 – Closed system of objects under operations`.
...
- With `raw=True`, `sample` returns the implementer's draw as produced (V.0): the kind's raw value for `sample_shape=()`, a `Distribution`- or `ConditionalDistribution`-valued draw being its own raw form and returning as the implementer's term, under its own name and provenance rather than fresh `sample` identity. For a non-empty `sample_shape` it returns the **storage view** (II.6), the draws' raw values in their native stacked layout:
...
The selected rule records its name and fidelity in the result's provenance wherever it is invoked, the direct call and the operation alike, mirroring the converter registry's recorded fidelity; `evaluate` adds only the operation's provenance record on top, and `method="..."` forces a rule by name. Rules need not be exact: an approximate scheme — quadrature, an unscented transform, quasi-Monte Carlo — registers at its recorded fidelity. Selection is fidelity-first: priorities are assigned from fidelity tiers, exact rules above approximate ones with the sampling lift the floor at the generic pair, and within a tier the more specific type pair wins, by the same closest-in-method-resolution-order tie-break the dispatch registries use, then registration order. Exactness is therefore never traded for type specificity silently: a rule that is more specific but approximate is selected only when no exact rule applies, or by `method=`. New rules join by registration, so an evaluation route for a new pair of types, exact or approximate, is added without touching the operation.
