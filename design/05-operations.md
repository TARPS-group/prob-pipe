...
The selected rule records its name and fidelity in the result's provenance wherever it is invoked, the direct call and the operation alike, mirroring the converter registry's recorded fidelity; `evaluate` adds only the operation's provenance record on top, and `method="..."` forces a rule by name. Rules need not be exact: an approximate scheme — quadrature, an unscented transform, quasi-Monte Carlo — registers at its recorded fidelity and ranks between the exact rules and the sampling floor. New rules join by registration, so an evaluation route for a new pair of types, exact or approximate, is added without touching the operation.
