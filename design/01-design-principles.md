...
**C5 — Naming for unambiguous meaning.** Each object and any of its components must carry semantically meaningful names, so a user can determine, from an object alone, what it and its components represent. Two sorts of name serve this, and neither substitutes for the other. An object's **label** is its name: given by the user at construction or derived by the operation that produced it, it describes the object, and only provenance and display read it, so neither matching nor dispatch nor compilation depends on it. A **component name** is a position in an interface: the name under which a produced value is offered to a consumer and matched against an input slot. A record's components are its fields, and any other term has exactly one component, which defaults to the object's label captured once at construction and may be declared otherwise; after construction the two names are independent. A function's results carry a further label, the function's result name, which is likewise descriptive. Labels and components are renamed by separate operations, so renaming an object never changes what a composition contains.
