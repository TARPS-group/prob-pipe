...
**Naming the result.** The joint's `name` is a label derived from the operands' labels and marked `name_is_auto`. Each operand contributes its label's **tokens**: an auto-derived label is a `·`-joined list of tokens, and a user-given label is one token. The tokens are listed in **canonical order**, which is the conditional-first topological order over the factors each token covers, with incomparable tokens ordered lexicographically by the fields their factors produce, and joined by `·`. So `lik * prior` is named `lik·prior`; `A * B * C`, `(A * B) * C`, and `A * (B * C)` are all named `A·B·C`; and `(lik * prior).with_name("posterior") * d` is named `posterior·d`, since the user's label stays one token.

Composition never reads a label or `name_is_auto`. Every operand enters as its flattened factors, so the joint's factors, its produced components, and its connections are the same whatever the operands are called: `AB * C` and `AB.with_name(AB.name) * C` are one joint under one name, and `(lik * prior).with_name("posterior") * d` is the three-factor joint of `lik`, `prior`, and `d`. The only effect of `with_name` on a later composition is the token it contributes to the derived label.
