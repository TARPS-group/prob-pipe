...
The name is marked `name_is_auto`. Selecting is not a step in a computation, so no provenance node records it: a selection carries the lineage of the batch it came out of, and which position it was is carried by the name, which states it exactly. A batch that *materializes* an element passes its own provenance to what it builds; a batch that *stores* its elements hands back the stored object, whose name and lineage are already its own — renaming it would mean handing back a copy, and an object placed in a batch by name already means something. A *sub-batch* takes the derived name whichever kind of storage it came from, since it is the batch's own view rather than an object the caller put there. Levels selected whole are left out, so selecting all of a batch derives the batch's own name; the levels that appear are listed in the batch's own order rather than the order they were indexed in. A derived name is therefore a function of what the view selects: two routes to the same selection read alike, and two different selections never do. A batch whose elements are bare values yields bare elements, which carry no identity to derive.
