...
Every tracked term exposes `raw()`, the **one door to the representation layer**: it returns the term **detached** from the workflow — no name, no lineage, nothing that would carry forward — and it is the only such door, so the escape hatch is one rule rather than a per-class convention. What detachment yields depends on whether the kind has a representation of its own to hand back. A kind with a raw host returns it: the backing array, the nested mapping of raw leaves, the wrapped callable, the wrapped object, an operator's stored parameterization, and, for a batch, the storage view. A kind whose representation *is* a ProbPipe object — a distribution, a kernel — returns a **detached copy**: the same mathematical object, no name, no provenance, no reference to the container it came from. Either way the rule a user learns is one sentence: `raw()` gives you the thing itself, cut loose from the workflow.
