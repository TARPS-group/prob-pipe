...
**Axis groups.** A batch's axes are partitioned into ordered **levels**: `axis_groups` tiles `batch_shape` into contiguous groups, outermost level first, and `batch_shape` stays their flat concatenation, so anything stated over `batch_shape`, flat vectorization above all, applies to a multi-level batch unchanged. A single-level batch has one group holding all its axes. Each level carries a **name**, with `level_names` giving one name per level in the same order. A name is auto-derived by the operation that produces the level and repinned with `with_level_names`, the batch-level counterpart of `with_path_names`: a shallow copy that changes only the level names, never the shapes or elements. Level names are unique within a batch: an operation that would mint a level whose canonical name the operand already carries disambiguates it deterministically, appending the smallest free integer suffix, so nested sampling yields `draw`, then `draw2`; a `with_level_names` rename onto a name already present raises, as field and factor renames do. The names identify the levels for display and, above all, for aligning batches, so an operation that combines batches matches their levels by name rather than by position, and two levels meant to correspond under different names are lined up by renaming one first, exactly as fields are realigned for composition. Level names name the multiplicity axes and are independent of any field names within each element, so a `RecordBatch` may carry a `draw` level over records whose fields are `mu` and `sigma` with no clash. `len`, `iter`, and indexing operate on the outermost level, and an element of a multi-level batch is the inner-level batch, as a view. Nesting therefore needs no dedicated classes: a batch is itself a tracked term, so a batch whose elements are batches is already admitted, and grouped storage presents the levels as views into one store. Nesting is unbounded in principle, though two levels, a collection of collections, cover the cases that arise in practice.
