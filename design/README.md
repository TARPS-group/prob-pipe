...
- **[Part III — Values and Distributions](03-values-and-distributions.md)** — the probability domain in dependency order: schemas, values, the function base, linear operators, distributions, conditional distributions, composition, and the hierarchy of distribution kinds, each with a precise contract that must align with the design principles.
