...
- **[Part II — Shared Abstractions](02-shared-abstractions.md)** — the generic, type-agnostic machinery the rest of the library is built on: the named-tree abstraction, the term-specification layer, identity, provenance, metadata, batching, and the dispatch registries.
- **[Part III — Values and Distributions](03-values-and-distributions.md)** — the probability domain in dependency order: the base value kinds with their specs, records and record batches, linear operators, distributions, conditional distributions, composition, and the hierarchy of distribution kinds, each with a precise contract that must align with the design principles.
