...
Class and method names are set in code font.  Design principles are cited only in the **Rationale** subsections, by identifier and short name, drawing on the *core principles* (the C-series) and *derived principles* (the D-series) of Part I. For example, the fourth core principle would be cited as `C4 – Function lifting via pushforward`. The class name `ConditionalDistribution` is used throughout, with *kernel* reserved for mathematical statements, such as defining a `ConditionalDistribution` as a probability kernel `K : S → P(T)`. 
