...
class Function(TrackedTerm):
    ...
    # evaluate the wrapped callable at a point (input_template -> output_spec),
...
Every value spec has a **batch form**, and every value spec has a tracked class (II.2). A `NumericArraySpec` value batches natively, as an array with the batch axes leading — but native storage is not identity, and the batch form is what carries a level name, a spec, and provenance. So the array kind takes `NumericArray` and `NumericArrayBatch` as the others do. Function-valued and opaque values have no native stacking either, so their `Batch` specializations provide it. Each batch is `Batch` over its element type and carries the shared spec its elements satisfy, adding no other interface.

`NumericArray` is one array value with identity: `Tracked` and `Annotated`, holding a single array whose `shape` is the **event** shape, with no batch axes. Its `raw` accessor is a view of that held array. It names the numeric-array kind alongside `NumericRecord`, whose leaves are values of that kind; `Array` stays the type alias for a bare backend array, so nothing is renamed to make room. It carries the full array surface — arithmetic, comparison, and the conversion hooks — because with no fields and no field count `arr + 1` has exactly one meaning. **Arithmetic yields a bare array, not a `NumericArray`.** Identity is attached by operations (V.0), and arithmetic is not one: a `NumericArray` is what an operation hands back and what a caller computes *from*. `Record` carries none of this surface and stays a container.

Multiplicity lives in `NumericArrayBatch`: a `Batch` whose `element_spec` is the `NumericArraySpec` and whose storage is one array with the batch axes leading — the same split `RecordBatch` uses, with one column instead of many. Its `raw()` is a view of that backing array. A `draw` level therefore has somewhere to live for an array-valued law (V.2).

`Opaque` adds identity and nothing else: no attribute forwarding, no `__call__`, one explicit `raw` accessor for the wrapped value.

Selection yields the tracked element kind for every batch: `NumericArrayBatch[i]` is a `NumericArray`, `FunctionBatch[i]` a `Function`, and `OpaqueBatch[i]` an `Opaque`. Array-backed and columnar batches materialize their elements under the derived selection identity and the batch's lineage.

`FunctionBatch` and `OpaqueBatch` use object-array storage. When a stored slot already contains the tracked element kind, indexing returns that exact object under its existing identity. When it contains only a raw callable or object, indexing wraps the selected value under the derived selection identity and the batch's lineage. A *sub-batch* remains a shared object-array view, and `raw()` returns that object array. Every element is checked against the shared `element_spec` at construction, reporting the position that failed, since one invalid element would make the batch's own spec a false statement.
...
- *Single-field presentation.* A `Record` is a container and presents as one, whatever its field count: no coercion, no arithmetic, no forwarding.
...
class RecordBatch(Batch[Record]):
    ...
    def raw(self) -> Mapping[str, Any]: ...          # nested mapping of raw column views
...
Storage is columnar: per-field columns in each field's batch form. A field column is therefore a direct tracked view, and an element `Record` is assembled on demand from the columns rather than stored a second time. `raw()` exposes the same store as a nested mapping whose leaves are the columns' raw storage views. A `RecordBatch` omits the field-keyed `Mapping` protocol (`keys()` / `values()` / `children`), so `len` and `iter` unambiguously range over the batch, and the field structure is read from `event_template`.
...
class Distribution[T](TrackedTerm, Annotated):
    ...
    @property
    def event_spec(self) -> TermSpec: ...       # view on spec: the event declaration
    @property
...
A `ConditionalDistribution` carries a `given_template`, the `EventTemplate` of the conditioning value `S`, and an `event_spec`, the declaration of one produced draw `T`. The event side is read exactly as for a `Distribution`: a bare `EventTemplate` wraps to `RecordSpec(template)` at construction, and the stored class fixes the draw kind. The given side is always an `EventTemplate`. Both sides are views on its stored `ConditionalDistributionSpec`, the single source of its type. Unlike a function's domain and codomain, a kernel's given and event are distinct *roles* — the value conditioned on versus the law produced — so their field names stay disjoint even when the two spaces coincide. For example, a Markov kernel (where `S = T`) uses names like `state → next_state` rather than `state → state`, for the same reason we write `K(x, dy)` rather than `K(x, dx)`. Symbolic dimensions are scoped over the two sides jointly, so a name shared between given and event fields is one dimension, and the fused conditional paths bind dimensions from the given value at call time. `with_path_names` renames fields across both sides, returning the same kernel under new field names, and `with_dims` binds symbolic dimensions across both.
...
**The conditional distribution value specification.** `ConditionalDistributionSpec` is the term spec of the `Cond` kind. As a leaf, it types a field holding a matching `ConditionalDistribution`. Its event side is a declaration, exactly as for `DistributionSpec`; the given side is always an `EventTemplate`:

```python
class ConditionalDistributionSpec(TermSpec):  # a ConditionalDistribution; is_valid accepts a match
    given_template: EventTemplate  # the conditioning schema, always a template
    event_spec: TermSpec           # the event declaration, as for DistributionSpec
```
...
The generic factored (conditional) distributions `FactoredDistribution` and `FactoredConditionalDistribution`  carry an ordered list of factors, each a `Distribution` or a `ConditionalDistribution`. The dependence graph is *derived* by matching each factor's given fields against the fields produced by earlier factors, rather than stored. The joint distribution's event declaration is the structural, disjoint union of the factors' produced templates: each factor's top-level fields become top-level fields of the joint, with their internal structure preserved and no additional nesting introduced. Factor names are unique across the list, and a duplicate is an error.
