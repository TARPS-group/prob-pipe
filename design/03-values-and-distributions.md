...
A `ConditionalDistribution[S, T]` is a *probability kernel* `K : S → P(T)` — a family of distributions p(· | s) indexed by a *conditioning value* `s : S`. Supply a value for what it conditions on and it yields an ordinary `Distribution` over what it produces. A `Distribution` is the special case of a kernel with an empty conditioning value — there is nothing to condition on — so the marginal law exists and `sample` / `log_prob` / `mean` apply unconditionally. A kernel with a non-empty given has no marginal law. The two are therefore distinct types, neither inheriting from the other: which unconditional operations an object carries turns on whether its given is empty, and the types mark the two cases. They are siblings sharing a capability vocabulary, each unconditional capability mirrored by a conditional twin that prepends the given.
