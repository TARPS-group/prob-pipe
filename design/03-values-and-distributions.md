...
A `ConditionalDistribution[S, T]` is a *probability kernel* `K : S → P(T)` — a family of distributions p(· | s) indexed by a *conditioning value* `s : S`. Supply a value for what it conditions on and it yields an ordinary `Distribution` over what it produces. A `Distribution` is the special case of a kernel with an empty conditioning value: the corner where the marginal law exists, so `sample` / `log_prob` / `mean` apply unconditionally. A kernel with a non-empty given has no such marginal, so those operations do not apply to it unconditionally. The two are kept as distinct types rather than one subclassing the other — a `ConditionalDistribution` is not a `Distribution` and does not inherit from one — precisely because the presence of the unconditional operations turns on whether the given is empty. They are thus siblings sharing a capability vocabulary, each unconditional capability mirrored by a conditional twin that shifts it by the given.
