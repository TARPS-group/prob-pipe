...
**The draw type `T`.** `T` is the implementer-side draw type, derived from the event spec's kind rather than declared independently: the spec is the source, and the bracket is typing documentation. Writing the tracked kind (`Distribution[NumericArray]`) or its raw host (`Distribution[Array]`) names the same array kind — either spelling is accepted and read at the kind level, the implementer receives the implementer type, the user receives the tracked draw, and raw access returns the stored representation, the two-presentation rule (II.1) applied to the type parameter. Per kind, the implementer type is the kind's raw host, except where the host cannot carry the structure the mathematics needs:
...
Conversion is also the entry route for raw distributions: under the two-presentation rule (II.1), a backend distribution supplied at a distribution-shaped position — an argument, a given, a record field — is accepted in its raw form and converted on entry through the registry, exactly as a bare array is wrapped at an array-spec position.
