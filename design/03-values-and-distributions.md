...
On any other path the exact route's guard declines before `_marginal` runs, leaving the fallback available when sampling is supported and the fidelity controls permit it (V.8).
...
**The family axis — how the law is realized.** Each family is an ordinary `Distribution`, or a `NumericDistribution` when its event is numeric, differing only in which capabilities it implements and how. The families themselves, with their events, capabilities, and how each arises, are Part VI's catalog.
