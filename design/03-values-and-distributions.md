...
A `FunctionSpec` types a callable by its input and output structure. Either template is optional: `None` leaves that side's structure unspecified, so a bare `FunctionSpec()` describes any callable, and the templates are supplied only where the signature is known or required. Each specified side is an explicit `EventTemplate` — a single-field signature is written out, e.g. `FunctionSpec(EventTemplate(x=...), EventTemplate(out=...))` — so a function's field names are always caller-chosen and meaningful, matching `DistributionSpec` (which likewise takes an explicit template).
...
A `LinOp` is a lazy linear map `A : ℝⁿ → ℝᵐ` between flat numeric spaces, and it is `Tracked`. It is how ProbPipe represents structured matrices, above all covariances, without materializing them. It carries an input and an output `NumericEventTemplate` (mirroring a `FunctionSpec`), so it maps numeric records and not just anonymous vectors. A `LinOp`'s templates name the numeric spaces it maps between, so it is built with those names — a bare matrix supplies them explicitly rather than defaulting to a single-field placeholder. Its two sides *may share a name*, in deliberate contrast to a `ConditionalDistribution` (whose given and event fields must stay disjoint, since a kernel's conditioning value and its draw are distinct random variables — `K(x, dy)`, not `K(x, dx)`): a self-adjoint operator, such as a covariance, a precision, or a Hessian, is an endomorphism on a single space, so its input and output templates are *equal*, and that shared name is what lets the operator algebra confirm that a composition lines up or that two operands act on the same space. A non-square operator such as a Jacobian names its two sides distinctly. Its templates are always concrete, and construction from a template with unbound dimensions raises. A consumer whose sizes are not yet known holds the operator as a recipe, the operator class and its size-free parameters, and mints the instance once the sizes are bound. The base fixes the action and the square-only queries, and every query raises `LinAlgError` where it is undefined:
