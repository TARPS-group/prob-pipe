...
The function kind's base type is `Function`: a tracked term wrapping exactly one Python callable. It carries the representation of a map: a `name`, `provenance`, and an input and an output `EventTemplate` — the two sides of its `FunctionSpec`, either side optional exactly as in the spec. Construction fixes all of it. The `input_template` is the function's authoritative interface: its top-level fields name the parameters, so binding an argument binds a field by name, and a signature is derived from it on demand rather than stored. When a plain callable is wrapped without an explicit template, its parameters seed those fields. The wrapped callable and its state stay private, reached only through the `Function`'s methods, so a stateful map exposes no backend object.
