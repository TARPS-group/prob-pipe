...
A `NumericDistribution` is a `Distribution` whose event spec is a `NumericSpec` (II.3), so its draws implement `Numeric` and the flat-vector machinery applies — a scalar `Normal`'s `NumericArraySpec` event qualifies exactly as a record event does.
...
- *The realigning `joint` form.* `joint(A, B, **align)` is `*` plus field renaming — path-valued targets included (II.6), so realignment can also promote a nested field to a slot-matchable name — for factors whose names do not line up: it is `A * B.with_path_names(**align)`.
