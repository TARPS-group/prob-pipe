...
`SupportsDifferentiation` is a claim an object carries, not a property read off its representation. A `Function` declares it through the `differentiable` flag; a `LinOp` and each distribution family that differentiates declare it the same way. A composite claims it when all of its parts do, so a joint, a transformed distribution, or a field view is differentiable exactly when the pieces it is built from are. An operation that needs gradients checks the claim and, when it is missing, fails at the step that lacks it rather than deep inside a backend trace. The claim is separate from execution dispatch: `jax` traces a call, but the claim, not the trace, decides whether gradients flow.
