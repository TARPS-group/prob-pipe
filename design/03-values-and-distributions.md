...
A `LinOp` is a lazy linear map `A : ℝⁿ → ℝᵐ` between flat numeric spaces, and it is `Tracked`. It is how ProbPipe represents structured matrices, above all covariances, without materializing them. It carries an input and an output `NumericEventTemplate` (mirroring a `FunctionSpec`), so it maps numeric records and not just anonymous vectors; those templates name the spaces it maps between, and a bare matrix is given names explicitly rather than defaulting to a single-field placeholder. Unlike a `ConditionalDistribution`'s given and event fields, which stay disjoint, a `LinOp`'s two sides *may share a name*: a self-adjoint operator (a covariance, a precision, a Hessian) maps a space to itself, so its input and output templates are equal, and that shared name is what lets the operator algebra check that operands compose or act on the same space. A non-square operator such as a Jacobian names its sides distinctly.
