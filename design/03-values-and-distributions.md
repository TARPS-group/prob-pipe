...
**The family axis — how the law is realized.** Each family is an ordinary `Distribution` (a `NumericDistribution` when its event is numeric), differing only in which capabilities it implements and how. The families themselves — their events, the capabilities each implements, and how each arises — are Part VI's catalog.
