...
__all__ = ["Record", "RecordTemplate", "NumericRecord"]
...
# ---------------------------------------------------------------------------
# NumericRecord — all-numeric-leaf Record with flatten/unflatten
# ---------------------------------------------------------------------------


def _is_numeric(val: Any) -> bool:
    """Check if a value is a numeric array (numpy or JAX) or numeric scalar."""
    ...
    if isinstance(val, (str, bytes, bool)):
        return False
    # Lists/tuples of numbers — will be converted by Record.__init__
...
class NumericRecord(Record):
    ...
    Adds ``flatten()`` / ``unflatten()`` for 1-D serialisation and
    stores a backend reference for array operations.
    ...
    **fields
    ...
    def __init__(
        ...
        **fields: ArrayLike | NumericRecord,
        ...
        # Validate all leaves are numeric
        self._validate_numeric()
        # Detect backend from first leaf
        from ._array_backend import detect_backend
        first_leaf = self._first_leaf()
        object.__setattr__(self, "_backend", detect_backend(first_leaf))
        ...
        for name in self._store:
            ...
            if isinstance(raw, Record):
                if not isinstance(raw, NumericRecord):
                    raise TypeError(
                        f"Field {name!r}: nested Record must be NumericRecord "
                        f"in a NumericRecord, got {type(raw).__name__}"
                    )
            elif not _is_numeric(raw):
                raise TypeError(
                    f"Field {name!r}: NumericRecord requires numeric values, "
                    f"got {type(raw).__name__}"
                )

    def _first_leaf(self) -> Any:
        """Return the first non-Record leaf value (for backend detection)."""
        for name in self._store:
            raw = self._store[name]
            if isinstance(raw, NumericRecord):
                return raw._first_leaf()
            return raw
        raise ValueError("NumericRecord has no leaves")  # pragma: no cover

    @property
    def backend(self):
        """The array backend (NumpyBackend or JaxBackend)."""
        return self._backend

    # -- Flatten / unflatten (override Record) --------------------------------
    ...
    def unflatten(
        ...
        *,
        ...
        template : Record or RecordTemplate
            Provides field names and shapes.  If a ``RecordTemplate``,
            uses the spec shapes directly.  If a ``Record``, uses
            resolved array shapes.
        """
        fields: dict[str, jnp.ndarray | NumericRecord] = {}
        offset = 0

        if isinstance(template, RecordTemplate):
            for name in template.fields:
                spec = template[name]
                if isinstance(spec, RecordTemplate):
                    size = spec.flat_size
                    child_flat = flat[offset:offset + size]
                    fields[name] = cls.unflatten(child_flat, template=spec)
                    offset += size
                elif spec is None:
                    raise TypeError(
                    ...
                    fields[name] = flat[offset:offset + size].reshape(spec)
                    offset += size
        else:
            # Legacy Record template
            for name in template._store:
                tval = template._resolve_field(name)
                if isinstance(tval, Record):
                    size = tval.flat_size
    ...
    def from_record(cls, record: Record) -> NumericRecord:
        ...
        raw_fields = {}
        for name in record._store:
            val = record._store[name]
            if isinstance(val, Record):
                raw_fields[name] = cls.from_record(val)
            else:
                raw_fields[name] = val
